...
export function exportNotesMarkdown(materia) {
  const sections = (materia.tarjetas || []).map((tarjeta) => {
    const meta = JSON.stringify({ title: tarjeta.titulo || "Apunte" });
...
export async function importNotesMarkdown(file) {
  ...
  while ((match = pattern.exec(markdown)) !== null) {
    let title = "Apunte importado";
    try {
      title = JSON.parse(match[1]).title || title;
    } catch {
      // Si los metadatos se editaron manualmente, conservamos un título seguro.
    }
    notes.push({
  ...
  const fallbackTitle = file.name.replace(/\.md$/i, "") || "Apunte importado";
  return [{
    id: `card-${Date.now()}`,
