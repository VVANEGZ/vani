...
export default function TipTapEditor({ content, onUpdate }) {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
      <div className="flex flex-wrap gap-1 border-b border-slate-200 bg-slate-50 p-1.5" aria-label="Formato de notas">
        ...
        <button type="button" title="Título 1" aria-label="Título 1" onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()} className={buttonClass(editor.isActive("heading", { level: 1 }))}>
          <Heading1 size={15} />
        </button>
        <button type="button" title="Título 2" aria-label="Título 2" onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} className={buttonClass(editor.isActive("heading", { level: 2 }))}>
          <Heading2 size={15} />
        </button>
        <button type="button" title="Título 3" aria-label="Título 3" onClick={() => editor.chain().focus().toggleHeading({ level: 3 }).run()} className={buttonClass(editor.isActive("heading", { level: 3 }))}>
          <Heading3 size={15} />
        </button>
        ...
        <button type="button" title="Añadir o editar enlace" aria-label="Añadir o editar enlace" onClick={setLink} className={buttonClass(editor.isActive("link"))}>
          <Link2 size={15} />
        </button>
        <button type="button" title="Quitar enlace" aria-label="Quitar enlace" disabled={!editor.isActive("link")} onClick={() => editor.chain().focus().unsetLink().run()} className={`${buttonClass(false)} disabled:cursor-not-allowed disabled:opacity-35`}>
          <Unlink size={15} />
        </button>
      </div>
